...
import type { TaskBatch, TaskBatchProgress } from '../../../common/tasks/schemas.js';
import { CreateRegionRequestWithGroupIdBody, EditRegionRequestWithIdBody } from "../../../common/tasks/schemas.js";
import { RegionTaskService } from "../service.js";
...
import { Region } from "../../regions/schemas.js";

export class RegionTaskWorkflowProcessor implements WorkflowAction {

	public constructor(
		private readonly log: FastifyBaseLogger,
		private readonly regionService: RegionService,
		private readonly regionTaskService: RegionTaskService,
		private readonly regionTaskItemService: TaskItemService,
	...
	public async process(batch: TaskBatch): Promise<void> {
		this.log.debug(`RegionTaskCreateWorkflow> process> task:${JSON.stringify(batch)}`);

		// update the overall task status to inProgress
		await this.regionTaskService.updateTaskStatus(batch.taskId, 'inProgress');

		// process task items
		const taskBatchProgress = await this.processTaskBatch(batch);

		// update task progress
		await this.regionTaskService.updateTaskProgress(taskBatchProgress);
		...
		await this.regionTaskService.updateTaskStatus(batch.taskId, 'success');

		this.log.debug(`RegionTaskCreateWorkflow> process> exit:${JSON.stringify(taskBatchProgress)}`);
	}

	private async processTaskBatch(batch: TaskBatch): Promise<TaskBatchProgress> {
		this.log.debug(`RegionTaskCreateWorkflow> processTaskItems> task:${JSON.stringify(batch)}`);
			...
			futures.push(
				limit(async () => {
					...
					try {
						...
						let actionResponse: Region;
						if (batch.type === 'create') {
							const { groupId, ...request } = a as CreateRegionRequestWithGroupIdBody;
							actionResponse = await this.regionService.create(batch.securityContext, groupId, request);
						} else if (batch.type === 'update') {
							const { id, ...request } = a as EditRegionRequestWithIdBody;
							actionResponse = await this.regionService.update(batch.securityContext, id, request);
						} else {
						...
						this.log.debug(`RegionTaskCreateWorkflow> processTaskItems> error: ${e.name}: ${e.message}`);
					}
		...
		await this.regionTaskItemService.createBulk(taskItems);
		...
		this.log.debug(`RegionTaskCreateWorkflow> processTaskItems> exit:${JSON.stringify(batchProgress)}`);
