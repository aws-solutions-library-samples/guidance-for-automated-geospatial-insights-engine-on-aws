...
class CloudGapFillProcessor(AbstractProcessor):
	...
	def process(self, stac_assets: Dataset) -> Dataset:
		if stac_assets.get('ndvi_raw') is None or stac_assets.get('scl_cloud_removed') is None:
			raise ValueError('ndvi_raw or scl_removed values is missing from Dataset')

		stac_assets["ndvi"] = XarrayUtils.fill_cloud_gap(stac_assets["scl_cloud_removed"], stac_assets['ndvi_raw'], self.previous_tif_raster)
