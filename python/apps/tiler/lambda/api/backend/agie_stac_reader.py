...
@cached(  # type: ignore
	...
	# the auth parameters could not be serialized to json, so we need to excluded that when constructing the LRU cache key
	key=lambda filepath, **kargs: hashkey(filepath, json.dumps({k: v for k, v in kargs.items() if k != "auth"})),
)
